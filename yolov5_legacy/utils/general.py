...
def ap_per_class(tp, conf, pred_cls, target_cls, plot=False, fname="precision-recall_curve.png"):
    """Compute the average precision, given the recall and precision curves. Source:
    https://github.com/rafaelpadilla/Object-Detection-Metrics. # Arguments tp: True positives (nparray, nx1 or
    nx10). conf: Objectness value from 0-1 (nparray). pred_cls: Predicted object classes (nparray). target_cls: True
    object classes (nparray). plot: Plot precision-recall curve at mAP@0.5 fname: Plot filename # Returns The
...
def compute_ap(recall, precision):
    """Compute the average precision, given the recall and precision curves. Source:
    https://github.com/rbgirshick/py-faster-rcnn. # Arguments recall: The recall curve (list). precision: The
    precision curve (list). # Returns The average precision as computed in py-faster-rcnn.
    """
    ...
    if method == "interp":
        ...
        # 替代 np.trapz 的手动计算
        # 梯形面积公式: sum((y[i+1] + y[i]) / 2 * (x[i+1] - x[i]))
        # 因为 x 是等间距的，dx = x[1] - x[0]
        dx = x[1] - x[0]
        # 计算 y_interp 的平均值（除了首尾）
        avg_y = (y_interp[:-1] + y_interp[1:]) / 2
        ap = np.sum(avg_y) * dx
    else:  # 'continuous'
...
def box_iou(box1, box2):
    ...
    """Return intersection-over-union (Jaccard index) of boxes. Both sets of boxes are expected to be in (x1, y1, x2,
    y2) format.
...
def strip_optimizer(f="weights/best.pt", s=""):  # from utils.general import *; strip_optimizer()
    ...
    x = torch.load(f, map_location=torch.device("cpu"), weights_only=False)  # 添加 weights_only=False
    x["optimizer"] = None
    ...
    print("Optimizer stripped from {},{} {:.1f}MB".format(f, (f" saved as {s},") if s else "", mb))
    ...
    for file in tqdm(sorted(glob.glob(f"{path}/*.*"))):
        ...
        if any(i):
            ...
            with open("new/labels/" + Path(file).name, "a") as f:  # write label
                f.writelines("{:g} {:.6f} {:.6f} {:.6f} {:.6f}\n".format(*tuple(l)) for l in labels[i])
...
def kmean_anchors(path="./data/coco128.yaml", n=9, img_size=640, thr=4.0, gen=1000, verbose=True):
    ...
    def print_results(k):
        ...
        print(
            f"n={n:g}, img_size={img_size}, metric_all={x.mean():.3f}/{best.mean():.3f}-mean/best, past_thr={x[x > thr].mean():.3f}-mean: ",
            end="",
        )
    ...
    if i:
        print(
            f"WARNING: Extremely small objects found. {i:g} of {len(wh0):g} labels are < 3 pixels in width or height."
        )
    wh = wh0[(wh0 >= 2.0).any(1)]  # filter > 2 pixels

    # Kmeans calculation
    print(f"Running kmeans for {n:g} anchors on {len(wh):g} points...")
    s = wh.std(0)  # sigmas for whitening
    k, _dist = kmeans(wh / s, n, iter=30)  # points, mean distance
    k *= s
...
def output_to_target(output, width, height):
    ...
    # --- 关键修改：确保整个output结构中的所有张量都移到CPU ---
    def move_tensors_to_cpu(obj):
        """递归地将对象中所有的torch.Tensor移动到CPU."""
    ...
    # 确保传入的output及其内部的所有张量都在CPU上
    output = move_tensors_to_cpu(output)
    # ---------------------------------------------------------

    # Convert model output to target format [batch_id, class_id, x, y, w, h, conf]
    # 注意：这里不再需要检查是否为torch.Tensor，因为我们已经在上面处理了
        ...
        if len(targets) > 0:
            ...
            for j, box in enumerate(boxes.T):
                ...
                if gt or conf[j] > 0.3:  # 0.3 conf thresh
                    label = f"{cls}" if gt else f"{cls} {conf[j]:.1f}"
...
def plot_study_txt(f="study.txt", x=None):  # from utils.general import *; plot_study_txt()
    ...
    _fig, ax = plt.subplots(2, 4, figsize=(10, 6), tight_layout=True)
    ax = ax.ravel()

    _fig2, ax2 = plt.subplots(1, 1, figsize=(8, 4), tight_layout=True)
    for f in [f"study/study_coco_yolov5{x}.txt" for x in ["s", "m", "l", "x"]]:
...
def plot_labels(labels, save_dir=""):
    ...
    try:
        ...
        sns.pairplot(
            ...
            kind="scatter",
            markers="o",
            plot_kws={"s": 3, "edgecolor": None, "linewidth": 1, "alpha": 0.02},
            diag_kws={"bins": 50},
        )
...
def plot_results(start=0, stop=0, bucket="", id=(), labels=(), save_dir=""):
    ...
    if bucket:
        ...
        files = [f"results{x:g}.txt" for x in id]
