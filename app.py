...
DATASET_INFO = {
    "name": "NEU-DET",
    "full_name": "Northeastern University Surface Defect Database",
    "description": "NEU-DET 是由东北大学发布的钢材表面缺陷数据集，广泛用于工业缺陷检测算法的训练与评估。"
    "数据集包含热轧带钢表面的 6 种典型缺陷类型，共计 1,800 张灰度图像。",
    "classes": 6,
    "total_samples": 1800,
    "image_size": "200×200 像素",
    "image_type": "灰度图 (8-bit)",
    "format": "PNG + XML标注 (Pascal VOC格式)",
    "source": "东北大学 (Northeastern University, China)",
    "year": 2013,
...
SAMPLE_IMAGES = {
    "crazing": "crazing_10.jpg",
    "inclusion": "inclusion_10.jpg",
    "patches": "patches_10.jpg",
    "pitted_surface": "pitted_surface_10.jpg",
    "rolled-in_scale": "rolled-in_scale_10.jpg",
    "scratches": "scratches_10.jpg",
}
...
def init_db():
    ...
    if not admin_exists:
        ...
        c.execute(
            "INSERT INTO users (username, password_hash, role) VALUES (?, ?, ?)",
            ("operator", hashlib.sha256(b"operator123").hexdigest(), "operator"),
        )
        c.execute(
            "INSERT INTO users (username, password_hash, role) VALUES (?, ?, ?)",
            ("viewer", hashlib.sha256(b"viewer123").hexdigest(), "viewer"),
        )
    # 尝试添加新列（兼容旧数据库）
    ...
    for f in sorted(os.listdir("models")) if os.path.exists("models") else []:
        if f.endswith(".pt"):
            ...
            if "yolov3" in nl:
                ...
                mtype = "YOLO26"
            elif "rtdetr" in nl or "rt-detr" in nl:
                mtype = "RT-DETR"
            elif "sam" in nl:
                mtype = "SAM"
            else:
                mtype = "自定义"
            models.append(
                {"name": f.replace(".pt", ""), "filename": f, "path": path, "type": mtype, "size_mb": size_mb}
            )
    return models
...
def _infer_yolov5(model_path, image_path, conf, iou):
    ...
    img = im0[:, :, ::-1]
    img, _ratio, _pad = letterbox(img, new_shape=img_size, auto=True)
    ...
    for det in pred:
        if det is not None and len(det):
            ...
            for *xyxy, c, cls_id in reversed(det):
                ...
                plot_one_box([x1, y1, x2, y2], annotated, label=label_text, color=(0, 255, 0), line_thickness=2)
...
def _load_and_infer(model_path, image_path, conf, iou):
    ...
    if _is_yolov5_checkpoint(model_path):
        xyxy, confs, cls, names, annotated_bgr, speed = _infer_yolov5(model_path, image_path, conf, iou)
...
def api_defect_sample_image(class_name):
    ...
    if not os.path.exists(img_path):
        return jsonify({"error": "样本图片不存在"}), 404
    ...
    if os.path.exists(xml_path):
        ...
        for obj in tree.getroot().iter("object"):
            ...
            cv2.putText(img_bgr, name, (xmin, max(ymin - 6, 12)), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
...
def api_model_compare():
    ...
    for m in models:
        try:
            t0 = time.time()
            xyxy, _confs, _cls, _names, _, _, mtype = _load_and_infer(m["path"], test_img, 0.25, 0.45)
    ...
    try:
        ...
        conn.execute(
            ...
            (
                ...
                avg_conf,
                infer_ms,
                json.dumps(defect_details, ensure_ascii=False),
    ...
    try:
        ...
        try:
            ...
            if "YOLOv5" in str(e) or "NOT forwards compatible" in str(e):
                use_v5 = True
                _import_yolov5_legacy()
                import importlib as _v5lib

                import torch

                def _v5i(m):
                    return _v5lib.import_module(m)
        ...
        for idx, img_file in enumerate(image_files):
            ...
            if use_v5:
                import importlib as _v5lib2

                import torch

                def _v5i2(m):
                    return _v5lib2.import_module(m)
                ...
                img_v5 = im0[:, :, ::-1]
                img_v5, _ratio, _pad = letterbox(img_v5, new_shape=640, auto=True)
                ...
                results = model.predict(
                    source=img_path, conf=conf_threshold, iou=iou_threshold, save=False, verbose=False
                )
                r = results[0]
                boxes = r.boxes
        ...
        return jsonify(
            {"task_id": task_id, "model_name": model_name, "batch": batch_results, "all_details": all_details}
        )
    except Exception as e:
...
def api_history():
    ...
    if role == "admin":
        ...
        rows = conn.execute(
            "SELECT * FROM detection_history ORDER BY id DESC LIMIT ? OFFSET ?", (per_page, offset)
        ).fetchall()
    else:
        total = conn.execute("SELECT COUNT(*) FROM detection_history WHERE username=?", (username,)).fetchone()[0]
        rows = conn.execute(
            "SELECT * FROM detection_history WHERE username=? ORDER BY id DESC LIMIT ? OFFSET ?",
...
def api_history_stats():
    ...
    if role == "admin":
        ...
        avg_infer = (
            conn.execute("SELECT AVG(infer_time_ms) FROM detection_history WHERE infer_time_ms > 0").fetchone()[0] or 0
        )
        all_details = conn.execute(
            "SELECT defect_details FROM detection_history WHERE defect_details != '[]'"
        ).fetchall()
    else:
        total_detections = conn.execute(
            "SELECT COUNT(*) FROM detection_history WHERE username=?", (username,)
        ).fetchone()[0]
        avg_infer = (
            conn.execute(
                "SELECT AVG(infer_time_ms) FROM detection_history WHERE infer_time_ms > 0 AND username=?", (username,)
            ).fetchone()[0]
            or 0
        )
        all_details = conn.execute(
            "SELECT defect_details FROM detection_history WHERE defect_details != '[]' AND username=?", (username,)
        ).fetchall()
    conn.close()
    class_counter = {}
    ...
    return jsonify(
        {
            ...
            "avg_infer_time_ms": round(avg_infer, 1),
